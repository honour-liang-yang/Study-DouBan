		...
		<template v-if="isComplete">
			...
			<form method="post" onsubmit='return false'>
				...
				<div class="form-alias">
					<label>
						<strong>token</strong>
						<input type="text" placeholder="token" v-model="token" name="token">
					</label>
				</div>
		...
		<template v-else>
			...
			<form method="post" @submit.prevent="onSubmit()">
				...
				<div class="form-alias">
					<label>
						<strong>邮箱</strong>
						<input type="email" placeholder="邮箱" v-model.trim="email" name="email">
					</label>
				</div>
				<div class="form-pwd">
					<label>
						<strong>密码</strong>
						<input :type="passType" placeholder="密码" v-model.trim="pass" name="pass">
				...
				<div class="form-name">
					<label>
						<strong>用户名</strong>
						<input type="text" placeholder="用户名" v-model.trim="name" name="name">
					</label>
				</div>
				<div>
					<button type="submit" class="submit" :disabled="isDisable" :class="{disabled:isDisable}">{{registerStatus}}</button>
				</div>
			</form>
			<div class="footer">
				<div class="argeement">点击「注册」代表你已阅读并同意用户使用协议</div>
				<div class="btns">
					<a href="#">下载豆瓣App</a>
				</div>
			</div>
...
<script>
	export default {
		...
		methods: {
			showPwd: function() {
				this.isShow = this.isShow ? 0 : 1 //0  false  1  true
				this.isShow ? this.passType = 'text' : this.passType = 'password'
			},
			beforeSubmit: function() {
				this.isDisable = true
				this.registerStatus = "正在提交"
			},
